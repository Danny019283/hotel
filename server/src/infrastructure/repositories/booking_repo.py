...
class Booking_repo(IRespository[Booking]):
    ...
    def room_has_bookings(self, room_number: int) -> bool:
        with Session(engine) as session:
            booking_room = session.exec(
                select(Booking_Room_model).where(Booking_Room_model.room_number == room_number)
            ).first()
            return booking_room is not None

    def occupied_ranges_for_room(self, room_number: int) -> list[tuple[int, object, object]]:
        with Session(engine) as session:
            booking_rooms = session.exec(
                select(Booking_Room_model).where(Booking_Room_model.room_number == room_number)
            ).all()
            ranges: list[tuple[int, object, object]] = []
            for booking_room in booking_rooms:
                booking = session.get(Booking_model, booking_room.booking_id)
                if booking is None:
                    continue
                if booking.check_out <= date.today():
                    continue
                ranges.append((booking.booking_id, booking.check_in, booking.check_out))
